...
<div class="row">
  ...
  <div class="col-md-3">
    <div class="form-group">
      ...
      <input type="date" class="form-control" id="date_of_rfq" name="date_of_rfq" value="<?php echo $view[0]['rfq_number'];?>" readonly="readonly">
    </div>
  </div>
...
<?php } } else {echo "<div class='alert alert-danger'>No Items Found</div>"; }?>

<div id="addmore"></div>

  <input type="button" name="btn"  class="btn btn-xs btn-secondary" value="Add Item From SHPL Catalogue" id="btn">

  <input type="button" name="btn" value="Client Design Add"  class="btn btn-rounded btn-info" data-toggle="modal" data-target="#modal-fill" onclick="show_page_model_fill('Add Client Design','<?php echo $base_url.'index.php?action=dashboard&nocss=sales_add_client_item&id='.$_GET['id'];?>')">

  <input type="button" onclick="form_submit('step0_edit')" name="Update" value="Save & Process" class="btn btn-secondary" id="submit_btn">
